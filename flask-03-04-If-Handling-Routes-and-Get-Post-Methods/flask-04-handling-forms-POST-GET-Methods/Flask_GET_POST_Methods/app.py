...
def calculate():
    if request.method == "POST":
        # request has come in from the form
        num1 = request.form.get("number1")
        num2 = request.form.get("number2")
        result = lcm(int(num1), int(num2))        
        return render_template("result.html", result1=num1, result2=num2, lcm=result, developer_name="Altaz")
    else:   # method = GET
        # user has hit this page directly
